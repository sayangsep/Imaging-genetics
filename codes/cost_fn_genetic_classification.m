function final_cost = cost_fn_genetic_classification(F,A,C,X,G,class,b,L_a,lambda1,lambda2,lambda3,lambda4)
...
o_2  = -2*sum( (class.*log(sigmoid(((X))'*b + eps))) + ((1-class).*log( (1-sigmoid(((X))'*b)) + eps ))  );
...
final_cost = o_1 + o_2 + o_4 + o_5 + o_7 + o_8  + o_10;
end
