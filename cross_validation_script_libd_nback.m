function [] = cross_validation_script_libd_nback(dd,ll1,ll2,ll3,ll4,idd)
...
%%  min_{A,X,b,B} ||F- A^T.X||^2  - Entropy(.)+ ||G - C^T.X||^2 ... 
 %              + lambda1/2*Tr(A*L_a*A')  + lambda5/2*(C*L_g*C')  +   lambda2*||C||_21 +
 %                lambda3/2*|| X ||_F
 %                lambda4/2*||b||^2
 
% F = V x nn, A = dim_X x 246, X_p = dim_X x n, b = dim_X x 1, G = g x nn, C = dim_X x g  
...
class_cv_hat       =    zeros([nn,length(dime),length(lambda1_c),...
                        length(lambda2_c),length(lambda3_c),length(lambda4_c)]);
                    
entropy_final      =    zeros([length(dime),length(lambda1_c),length(lambda2_c),...
                        length(lambda3_c),length(lambda4_c)]);
                    
overlap_final      =    zeros([length(dime),length(lambda1_c),length(lambda2_c),...
                        length(lambda3_c),length(lambda4_c)]);
...
parfor c = 1:fold+1 % parallelize the CV folds.
...
cost_fn_genetic_classification(F_train,A_temp,C,X,G_train,class_train,b,L_a,lambda1,lambda2,lambda3,lambda4)
...
cost_fn_genetic_classification(F_train,A_temp,C_temp,X,G_train,class_train,b,L_a,lambda1,lambda2,lambda3,lambda4)


%%  min_{X}       ||F- A^T.X||^2  - Entropy(.) + ||G - C^T.X||^2 + ... 
%                 lambda3/2|| X ||_F
X_temp = optimize_X_imaging_genetics(F_train,A_temp,X,G_train,C_temp,class_train,b,lambda3); 

cost_fn_genetic_classification(F_train,A_temp,C_temp,X_temp,G_train,class_train,b,L_a,lambda1,lambda2,lambda3,lambda4) 
             

%%  min_{A,X,b,B} -Entropy(.) ...
 %                lambda4/2*||b||^2  
b_temp = optimize_b(X_temp,class_train,b,lambda4);

cost_fn_genetic_classification(F_train,A_temp,C_temp,X_temp,G_train,class_train,b_temp,L_a,lambda1,lambda2,lambda3,lambda4)

%% Continue from here. Need to send groups for groups sparsity regularization calculation.    
y(iter) = cost_fn_genetic_classification(F_train,A_temp,C_temp,X_temp,G_train,class_train,b_temp,L_a,lambda1,lambda2,lambda3,lambda4);
...
% figure(12),subplot(1,5,2),imagesc(A_temp*(F_p_train-s_temp)),colormap('jet'),caxis([min(X(:)) max(X(:))]),colorbar,title('classification');
% figure(12),subplot(1,5,4),imagesc(b),colormap('jet'),colorbar,title('b');
% figure(12),subplot(1,5,5),imagesc(A'),colormap('jet'),colorbar,title('A^T');
...
% figure(6), plot(ppp(:),ss(:),'o',ppp(:),zz(:),'*');
% xlim([min([min(ppp(:)),min(ss(:))]) max( [max(ppp(:)),max(ss(:))] )]); 
% ylim([min([min(ppp(:)),min(ss(:))]) max( [max(ppp(:)),max(ss(:))] )]);  xlabel('original prs');ylabel('predicted prs');
% title(sprintf('CV dim = %s, l_0=%s, l_1=%s,l_2=%s',num2str(dim),num2str(lambda0),num2str(lambda1),num2str(lambda2)));
...
class_cv_hat(:,dd,ll1,ll2,ll3,ll4) = ss(:)';

entropy_final(dd,ll1,ll2,ll3,ll4) =  -(sum( (class(:).*log(ss(:))) + ((1-class(:)).*log( (1-ss(:)) ))  ));

overlap_final(dd,ll1,ll2,ll3,ll4) = perf;
